...
def setup_hdfs(HA):
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        if not HA:
			...
			edge_host_id_1=[host for host in hosts if host.id == 3][0]  #--TSE
			edge_host_id_2=[host for host in hosts if host.id == 4][0]  #--TSE
			TSE_nodes=[nn_host_id,snn_host_id,master_host_id_3,edge_host_id_1,edge_host_id_2]'''
    ...
    for role_type in ['DATANODE']:
        for host in management.get_hosts(include_cm_host = False):
			...
			if host.hostId not in TSE_nodes_master and not in TSE_nodes_edge: #--TSE
        ...
        for role_type in ['GATEWAY']:
            for host in management.get_hosts(include_cm_host=(role_type == 'GATEWAY')):
                '''if host.hostId != nn_host_id.hostId:
					if host.hostId != snn_host_id.hostId:
						if role_type != 'DATANODE':
							cdh.create_service_role(service, role_type, host)'''
					if host.hostId in TSE_nodes_edge:  #--TSE
						cdh.create_service_role(service, role_type, host)	#--TSE 
...
def setup_spark_on_yarn():
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        #cdh.create_service_role(service, "SPARK_YARN_HISTORY_SERVER",cmhost) #--TSE
		
		cdh.create_service_role(service, "SPARK_YARN_HISTORY_SERVER",[host for host in hosts if host.id == 1][0])
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        for rcg in [x for x in service.get_all_role_config_groups()]:
            ...
            if rcg.roleType == "JOBHISTORY":
               ...
               # cdh.create_service_role(service, rcg.roleType, cmhost)
			   cdh.create_service_role(service, rcg.roleType, rm_host_id)  #TSE
                ...
                for host in management.get_hosts(include_cm_host = False):
                    ...
                    '''    if host.hostId != rm_host_id.hostId:
                            if host.hostId != srm_host_id.hostId:
                                cdh.create_service_role(service, role_type, host)''' #TSE
					if host.hostId not in TSE_nodes_master and not in TSE_nodes_edge:
						cdh.create_service_role(service, role_type, host)
...
def setup_hive():
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        for role_type in ['HIVEMETASTORE', 'HIVESERVER2']:
            cdh.create_service_role(service, role_type, [host for host in hosts if host.id == 2][0])  #TSE
...
def setup_sqoop():
    ...
    if cdh.get_service_type(service_type) is None:
        ...
        #install to CM node, mingrui
        cmhost= management.get_cmhost()
        #cdh.create_service_role(service, "SQOOP_SERVER", cmhost) #TSE
		cdh.create_service_role(service, "SQOOP_SERVER", [host for host in hosts if host.id == 2][0])
...
def setup_kafka():   #TSE
    ...
    if cdh.get_service_type(service_type) is None:
		...
		cdh.create_service_role(service, "KAFKA_SERVER", [host for host in hosts if host.id == 2][0])
	    
        check.status_for_command("Creating kafka user directory", service.create_kafka_user_dir())
        # This service is started later on
        # check.status_for_command("Starting kafka Service", service.start())
		
def setup_kafka_client():  #TSE
    """
    kafka Client
    :return:
    """
    ...
    try:
        ...
        if len(hdfs.get_roles_by_type("NAMENODE")) != 2:
            ...
            '''cmd = hdfs.enable_nn_ha(hdfs.get_roles_by_type("NAMENODE")[0].name, standby_host_id,
									...
									# TSE						
			cmd = hdfs.enable_nn_ha(hdfs.get_roles_by_type("NAMENODE")[0].name, standby_host_id,
                                    "nameservice1", [dict(jnHostId=nn_host_id), dict(jnHostId=sndnn_host_id), dict(jnHostId=master_host_id_3)],zk_service_name=zookeeper.name)
...
def setup_sentry():
    ...
    if cdh.get_service_type(service_type) is None:
		...
		cdh.create_service_role(service, "SENTRY_SERVER", [x for x in hosts if x.id == 2][0])
...
def main():
	...
	global hosts,nn_host_id,snn_host_id,master_host_id_3,edge_host_id_1,edge_host_id_2,TSE_nodes  #--TSE
