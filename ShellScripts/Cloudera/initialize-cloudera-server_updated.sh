...
if $HA; then
    ...
    echo "$ClusterName -u $User -p $Password -m $mip -w $worker_ip -a -c $cmUser -s $cmPassword -e -r $EMAILADDRESS -b $BUSINESSPHONE -f $FIRSTNAME -t $LASTNAME -o $JOBROLE -i $JOBFUNCTION -y $COMPANY -v $VMSIZE"
        status=$?
else
